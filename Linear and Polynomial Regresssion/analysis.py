...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        '''
        min = np.min(self.data.select_data(headers, rows), 0)
    ...
    def max(self, headers, rows=[]):
        ...
        max = np.max(self.data.select_data(headers,rows),0)
        return max
        pass
    ...
    def mean(self, headers, rows=[]):
        ...
        '''
        # sum of each column
        sum = np.sum(self.data.select_data(headers,rows), 0)
        mean = np.divide(sum, self.data.select_data(headers,rows).shape[0])
        return mean
        pass
    ...
    def var(self, headers, rows=[]):
        ...
        x = (np.subtract(self.data.select_data(headers, rows),mean))**2
        sum = np.sum(x, 0)
       
        var = np.divide(sum, x.shape[0])
    ...
    def std(self, headers, rows=[]):
        ...
        std = np.sqrt(np.divide(sum,x.shape[0]))
        return std
        pass
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        # makes subplots with N rows and N columns, all the subplots share x and y axes
        ...
        for i in range(len(data_vars)):
            for j in range(len(data_vars)):
                axes[i][j].scatter(data[:, j], data[:, i], marker=".")
    ...
    # EXTENSION - BAR CHART DATA VISUALIZATION
    def bar(self, ind_var, figsize,title):

        '''Creates a bar plot with "x" variable in the dataset `ind_var` and
        "y" variable which is the mean of each header category

        Parameters:
        -----------
        ind_var: str.
            List of headers that is plotted along the x axis
        figsize: tuple
        ...
        means = self.mean(ind_var)
        fig = plt.figure(figsize = figsize)
        bar = plt.bar(ind_var,list(means), color ='green')
        plt.title(title)
