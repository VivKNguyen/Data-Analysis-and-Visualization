...
class KMeans:
    ...
    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False):
        ...
        while count < max_iter :
            ...
            # print(self.data_centroid_labels[0:1000:100])
            ...
            if verbose == True:
                ...
                print('current diff: ', centroid_diff)
                print('centroid_diff - tol: ', centroid_diff - tol)
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False):
        ...
        best_centroid = np.zeros((n_iter, k, self.data.shape[1]))
        best_label = np.zeros((n_iter, self.data.shape[0]), dtype =int)
        best_inertia = np.zeros((n_iter))
        
        for i in range( n_iter):
            self.cluster(k)
            
            best_inertia[i] = self.inertia
            best_centroid[i] = self.centroids
            best_label[i] = self.data_centroid_labels

            if verbose == True:
                print('inertia: ', best_inertia)
        
        min_idx = np.argmin(best_inertia)
        
        # print(best_inertia)
        
        if verbose == True:
            print('inertia: ', best_inertia)
            print('labels: ', self.data_centroid_labels)
            print('Min index: ', min_idx)
            
        self.centroids = best_centroid[min_idx]
        self.data_centroid_labels = best_label[min_idx]
        # print((self.data_centroid_labels))
        self.inertia = best_inertia[min_idx]
    ...
    def update_labels(self, centroids):
        ...
        # get the distances from each data point to each centroid
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        centroid_diff = np.zeros((k,prev_centroids.shape[1]))
        for i in range(k):

            samples_in_cluster = (data_centroid_labels == i)

            if np.sum(samples_in_cluster) > 0:

                new_centroids[i] = np.mean(self.data[samples_in_cluster], axis=0)
    ...
    def compute_inertia(self):
        ...
        for i in range(self.num_samps):
            cent_idx = self.data_centroid_labels[i]
            # print(self.data[i])
            # print(self.centroids[cent_idx])
            dist = np.linalg.norm(self.data[i] - self.centroids[cent_idx])**2
    ...
    def plot_clusters(self):
        ...
        plt.scatter(self.data[:,0], self.data[:,1], c = self.data_centroid_labels, cmap = 'Blues', edgecolors='black')
    ...
    def elbow_plot(self, max_k, n_iter = 1):
        ...
        n_iter: int. Number of iterations that cluster_batch will go through
        ...
        xaxis= np.arange(1,max_k +1)
       
        
        for i in range (1,max_k+1):
            self.cluster_batch(i, n_iter)
            inertias_list.append(self.inertia)
        


        
        plt.plot(xaxis, inertias_list)
